...
public class CheckpointingOptions {
    ...
    public static final ConfigOption<Boolean> RESTORE_WITH_RESCALING_BY_FILE_INGESTION =
            ConfigOptions.key("state.backend.restore-with-rescaling.file-ingestion")
                    .defaultValue(false);

    public static final ConfigOption<Boolean> PARALLELIZED_L0_COMPACTION_FOR_FILE_INGESTION =
            ConfigOptions.key("state.backend.restore-with-rescaling.parallelized-L0-compaction")
                    .defaultValue(false);

    public static final ConfigOption<Boolean> FAKE_RESTORE_PREPARE =
            ConfigOptions.key("state.backend.restore.fake-restore-prepare")
                    .defaultValue(false);

    public static final ConfigOption<Boolean> FAKE_RESTORE =
            ConfigOptions.key("state.backend.restore.fake-restore")
                    .defaultValue(false);
}
