 ...
 * backend define how to hold the working state for keys and operators. They also define how to
 * checkpoint that state, frequently using the raw bytes storage (via the {@code
...
public interface StateBackend extends java.io.Serializable {
    Logger LOG = LoggerFactory.getLogger(StateBackend.class);
    ...
    default void startCompactionScheduler(Supplier<Boolean> keyEliminationCallback)
            throws IOException {
    }

    default CompactionCollector getCompactionCollector() {
        return null;
    }

    default boolean isEnableRestoreWithRescalingByFileIngestion() {
        return false;
    }
}
