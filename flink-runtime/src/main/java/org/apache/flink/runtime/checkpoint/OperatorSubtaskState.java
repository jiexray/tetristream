...
public class OperatorSubtaskState implements CompositeStateHandle {
    ...
    @Nonnull private final StateObjectCollection<OperatorStateHandle> managedOperatorState;

    /**
     * Snapshot written using {@link
     * org.apache.flink.runtime.state.OperatorStateCheckpointOutputStream}.
     */
    @Nonnull private final StateObjectCollection<OperatorStateHandle> rawOperatorState;

    /** Snapshot from {@link org.apache.flink.runtime.state.KeyedStateBackend}. */
    @Nonnull private final StateObjectCollection<KeyedStateHandle> managedKeyedState;

    /**
     * Snapshot written using {@link
     * org.apache.flink.runtime.state.KeyedStateCheckpointOutputStream}.
     */
    @Nonnull private final StateObjectCollection<KeyedStateHandle> rawKeyedState;

    @Nonnull private final StateObjectCollection<InputChannelStateHandle> inputChannelState;
    ...
    /**
     * For convenience because the size of the collections is typically 0 or 1. Null values are
     * translated into empty Collections.
     */
    public OperatorSubtaskState(
    ...
    private static <T extends StateObject> StateObjectCollection<T> singletonOrEmptyOnNull(
            T element) {
        return element != null
                ? StateObjectCollection.singleton(element)
                : StateObjectCollection.empty();
    }
    ...
    public long getTotalStateSize() {
        long calculateStateSize = managedOperatorState.getTotalStateSize();
        calculateStateSize += rawOperatorState.getTotalStateSize();
        calculateStateSize += managedKeyedState.getTotalStateSize();
        calculateStateSize += rawKeyedState.getTotalStateSize();
