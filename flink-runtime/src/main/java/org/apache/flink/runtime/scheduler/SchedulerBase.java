...
public abstract class SchedulerBase implements SchedulerNG, CheckpointScheduling {
    ...
    private ExecutionGraph createAndRestoreExecutionGraph(
        ...
        ExecutionGraph newExecutionGraph =
                createExecutionGraph(
                        currentJobManagerJobMetricGroup, shuffleMaster, partitionTracker);
    ...
    @Override
    public void acknowledgeKeyElimination(JobID jobID, ExecutionAttemptID executionAttemptID) {
        mainThreadExecutor.assertRunningInMainThread();

        final CheckpointCoordinator checkpointCoordinator =
                executionGraph.getCheckpointCoordinator();
        final KeyEliminationMessage ackMessage =
                new KeyEliminationMessage(jobID, transferExecutionAttemptID(executionAttemptID));
        ...
        if (checkpointCoordinator != null) {
            ioExecutor.execute(
                    () -> {
                        try {
                            checkpointCoordinator.receiveKeyEliminationAcknowledgement(
                                    ackMessage, taskManagerLocationInfo);
                        } catch (Throwable t) {
                        }
                    });
    ...
    private ExecutionVertexID transferExecutionAttemptID(ExecutionAttemptID executionAttemptID) {
