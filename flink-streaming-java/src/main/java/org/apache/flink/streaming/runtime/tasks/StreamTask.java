...
public abstract class StreamTask<OUT, OP extends StreamOperator<OUT>> extends AbstractInvokable
    ...
    protected void beforeInvoke() throws Exception {
        ...
        actionExecutor.runThrowing(
                () -> {
                    ...
                    // WARN: should be done before operatorChain.initializeStateAndOpenOperators
                    // (see FLINK-19907)
                    ...
                    if (reader.hasChannelStates()) {
                        ...
                        // It would get possible benefits to recovery input side after output side,
                        ...
                        if (inputGates != null && inputGates.length > 0) {
                            ...
                            // Note that we must request partition after all the single gates
                            // finished recovery.
                                    ...
                                    .thenRun(
                                            () ->
                                                    mainMailboxExecutor.execute(
                                                            ...
                                                            "Input gates request partitions"));
                        }
                    ...
                    if (stateBackend.getCompactionCollector() != null) {
                        registerCompactionScheduler(
                                getEnvironment().getTaskInfo(), stateBackend.getCompactionCollector());

                        Supplier<Boolean> keyEliminationCallback =
                                stateBackend.isEnableRestoreWithRescalingByFileIngestion()
                                        ? this::keyEliminationReportCallback
                                        : null;
                        stateBackend.startCompactionScheduler(keyEliminationCallback);
                        cancelables.registerCloseable(stateBackend.getCompactionCollector());
                    }
                });
    ...
    protected void cleanUpInvoke() throws Exception {
        ...
        unregisterCompactionScheduler(getEnvironment().getTaskInfo());
    ...
    public boolean keyEliminationReportCallback() {
        Preconditions.checkArgument(stateBackend.isEnableRestoreWithRescalingByFileIngestion());
        getEnvironment().getTaskStateManager().reportKeyElimination();
        return true;
    }
}
