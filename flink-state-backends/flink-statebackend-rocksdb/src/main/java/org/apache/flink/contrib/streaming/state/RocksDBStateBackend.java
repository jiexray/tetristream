...
public class RocksDBStateBackend extends AbstractStateBackend implements ConfigurableStateBackend {
    ...
    @Nullable private transient CompactionScheduler compactionScheduler;

    private transient boolean enableRestoreWithRescalingByFileIngestion = false;

    private transient boolean parallelizedL0Compaction = false;

    private transient boolean fakeRestorePrepare = false;

    private transient boolean fakeRestore = false;
    ...
    private RocksDBStateBackend(
        ...
        enableRestoreWithRescalingByFileIngestion =
                config.get(CheckpointingOptions.RESTORE_WITH_RESCALING_BY_FILE_INGESTION);
        parallelizedL0Compaction =
                config.get(CheckpointingOptions.PARALLELIZED_L0_COMPACTION_FOR_FILE_INGESTION);
        if (!enableRestoreWithRescalingByFileIngestion) {
            parallelizedL0Compaction = false;
        }

        fakeRestore = config.get(CheckpointingOptions.FAKE_RESTORE);
        fakeRestorePrepare = config.get(CheckpointingOptions.FAKE_RESTORE_PREPARE);
        ...
        if (compactionScheduler != null) {
            this.predefinedOptions = PredefinedOptions.COMPACTION_DISABLED;
        }
    ...
    @Override
    public void startCompactionScheduler(Supplier<Boolean> keyEliminationCallback) {
        if (compactionScheduler != null) {
            compactionScheduler.finishRestore(keyEliminationCallback);
        } else {
            Preconditions.checkArgument(
                    keyEliminationCallback != null,
                    "if enable restore-with-rescaling-by-file-ingestion, must open compaction scheduler.");
        }
    }

    @Nullable
    @Override
    public CompactionCollector getCompactionCollector() {
        return compactionScheduler;
    }

    @Override
    public boolean isEnableRestoreWithRescalingByFileIngestion() {
        return enableRestoreWithRescalingByFileIngestion;
    }

    public boolean isParallelizedL0Compaction() {
        return parallelizedL0Compaction;
    }
    ...
    public <K> AbstractKeyedStateBackend<K> createKeyedStateBackend(
        ...
        final OpaqueMemoryResource<RocksDBSharedResources> sharedResources =
                RocksDBOperationUtils.allocateSharedCachesIfConfigured(
                        memoryConfiguration, env.getMemoryManager(), LOG);
        ...
        RocksDBKeyedStateBackendBuilder<K> builder =
                new RocksDBKeyedStateBackendBuilder<>(
                        ...
                        .setCompactionScheduler(compactionScheduler)
                        .setEnableRestoreWithRescalingByFileIngestion(enableRestoreWithRescalingByFileIngestion)
                        .setParallelizedL0Compaction(parallelizedL0Compaction)
                        .setEnableFakeRestore(fakeRestore)
                        .setFakeRestorePrepare(fakeRestorePrepare);
