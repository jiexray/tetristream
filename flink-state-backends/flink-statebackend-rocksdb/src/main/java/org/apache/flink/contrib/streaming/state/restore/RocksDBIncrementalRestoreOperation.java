...
public class RocksDBIncrementalRestoreOperation<K> extends AbstractRocksDBRestoreOperation<K> {
    ...
    public IncrementalLocalKeyedStateHandle transferRemoteStateToLocalDirectory(
    ...
    private void restoreWithRescaling(Collection<KeyedStateHandle> restoreStateHandles)
        ...
        for (KeyedStateHandle rawStateHandle : restoreStateHandles) {
            ...
            if (!(rawStateHandle instanceof IncrementalRemoteKeyedStateHandle ||
                    rawStateHandle instanceof  IncrementalLocalKeyedStateHandle)) {
            ...
            if (rawStateHandle instanceof IncrementalRemoteKeyedStateHandle) {
                ...
                temporaryRestoreInstancePath = ((IncrementalLocalKeyedStateHandle) rawStateHandle).getDirectoryStateHandle().getDirectory();
            }
    ...
    private void initDBWithRescaling(KeyedStateHandle initialHandle) throws Exception {
        ...
        } else if (initialHandle instanceof IncrementalLocalKeyedStateHandle) {
            restoreFromLocalState((IncrementalLocalKeyedStateHandle) initialHandle);
        } else {
            throw new Exception("Unknown type of initialHandle: " + initialHandle.getClass());
        }
    ...
    private RestoredDBInstance restoreDBInstanceFromStateHandle(
            KeyedStateHandle restoreStateHandle, Path temporaryRestoreInstancePath)
            throws Exception {
        if (restoreStateHandle instanceof IncrementalRemoteKeyedStateHandle) {
            try (RocksDBStateDownloader rocksDBStateDownloader =
                         new RocksDBStateDownloader(numberOfTransferringThreads)) {
                rocksDBStateDownloader.transferAllStateDataToDirectory(
                        (IncrementalRemoteKeyedStateHandle) restoreStateHandle, temporaryRestoreInstancePath, cancelStreamRegistry);
            }
        }

        KeyedBackendSerializationProxy<K> serializationProxy =
                restoreStateHandle instanceof IncrementalRemoteKeyedStateHandle ?
                    readMetaData(((IncrementalRemoteKeyedStateHandle) restoreStateHandle).getMetaStateHandle()) :
                        readMetaData(((IncrementalLocalKeyedStateHandle) restoreStateHandle).getMetaDataState());
        // read meta data
        List<StateMetaInfoSnapshot> stateMetaInfoSnapshots =
                serializationProxy.getStateMetaInfoSnapshots();
